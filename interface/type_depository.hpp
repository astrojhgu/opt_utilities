...
namespace opt_utilities
{
    ...
    class type_unregistered : public opt_exception
    {
    ...
    template <typename T> class holder
    {
      ...
      public:
        ...
        holder (const holder &rhs) : ptr (rhs.ptr)
        {
    ...
    template <typename Ty, typename Tx> void delete_clone (const func_obj<Ty, Tx> *pfo)
    {
    ...
    void fetch_func_obj (const func_obj<Ty, Tx> *&fo, std::string cname, fetch_direction dir = in)
    {
        ...
        typename std::map<std::string, holder<func_obj<Ty, Tx>>>::iterator it = pm.find (cname);
    ...
    template <typename Ty, typename Tx> void register_func_obj (const func_obj<Ty, Tx> &fo)
    {
        const func_obj<Ty, Tx> *pfo = &fo;
        fetch_func_obj (pfo, fo.get_type_name (), in);
    }

    template <typename Ty, typename Tx> const func_obj<Ty, Tx> *get_func_obj (std::string cname)
    {
    ...
    template <typename Ty, typename Tx> void delete_clone (const opt_method<Ty, Tx> *pfo)
    {
    ...
    void fetch_opt_method (const opt_method<Ty, Tx> *&fo, std::string cname, fetch_direction dir)
    {
        ...
        typename std::map<std::string, holder<opt_method<Ty, Tx>>>::iterator it = pm.find (cname);
    ...
    template <typename Ty, typename Tx> void register_opt_method (const opt_method<Ty, Tx> &fo)
    {
        const opt_method<Ty, Tx> *pfo = &fo;
        fetch_opt_method (pfo, fo.get_type_name (), in);
    }

    template <typename Ty, typename Tx> const opt_method<Ty, Tx> *get_opt_method (std::string cname)
    {
    ...
    void fetch_statistic (const statistic<Tdata, Tp, Ts, Tstr> *&fo, std::string cname, fetch_direction dir)
    {
        ...
        typename std::map<std::string, holder<statistic<Tdata, Tp, Ts, Tstr>>>::iterator it = pm.find (cname);
    ...
    void fetch_param_modifier (const param_modifier<Tdata, Tp, Tstr> *&fo, std::string cname, fetch_direction dir)
    {
        ...
        typename std::map<std::string, holder<param_modifier<Tdata, Tp, Tstr>>>::iterator it = pm.find (cname);
    ...
    template <typename Tdata> void delete_clone (const data_set<Tdata> *pfo)
    {
    ...
    void fetch_data_set (const data_set<Tdata> *&fo, std::string cname, fetch_direction dir)
    {
        ...
        typename std::map<std::string, holder<data_set<Tdata>>>::iterator it = pm.find (cname);
    ...
    template <typename Tdata> void register_data_set (const data_set<Tdata> &fo)
    {
        const data_set<Tdata> *pfo = &fo;
        fetch_data_set (pfo, fo.get_type_name (), in);
    }

    template <typename Tdata> const data_set<Tdata> *get_data_set (std::string cname)
    {
    ...
    void fetch_model (const model<Tdata, Tp, Tstr> *&fo, std::string cname, fetch_direction dir)
    {
        ...
        typename std::map<std::string, holder<model<Tdata, Tp, Tstr>>>::iterator it = pm.find (cname);
