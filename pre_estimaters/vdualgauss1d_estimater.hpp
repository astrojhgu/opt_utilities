...
{
    template <typename T>
    class dualgauss1d_estimater : public pre_estimater<data<optvec<T>, optvec<T>>, optvec<T>, std::string>
    {
      public:
        ...
        void do_estimate (const data_set<data<optvec<T>, optvec<T>>> &d,
                          model<data<optvec<T>, optvec<T>>, optvec<T>, std::string> &m) const
        {
            int n = d.size ();
