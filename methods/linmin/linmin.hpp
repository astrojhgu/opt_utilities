...
{
    template <typename rT, typename pT> class func_adaptor : public func_obj<rT, rT>
    {
      private:
        const pT p1, xi1;
        ...
        func_adaptor (const func_adaptor &) : func_obj<rT, rT> (), p1 (), xi1 (), pfoo (NULL_PTR)
        {
        }
            ...
            for (size_t i = 0; i < get_size (xt); ++i)
                {
                    ...
                    set_element (xt, i, get_element (xt, i) + x * get_element (xi1, i));
        ...
        for (j = 0; j < n; ++j)
            {
                ...
                set_element (xi, j, get_element (xi, j) * xmin);
                // get_element(p,j)+=get_element(xi,j);
                set_element (p, j, get_element (p, j) + get_element (xi, j));
            }
        //  delete xicom_p;
        // delete pcom_p;
    }
