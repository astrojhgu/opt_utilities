...
namespace opt_utilities
{
    ...
    template <typename rT, typename pT> struct vp_pair
    {
        rT v;
        pT p;
    };

    template <typename rT, typename pT> class vp_comp
    {
      public:
        bool operator() (const vp_pair<rT, pT> &x1, const vp_pair<rT, pT> &x2)
        {
            return x1.v < x2.v;
        }
    };
     ...
     */
    template <typename rT, typename pT> class aga_method : public opt_method<rT, pT>
    {
      private:
        typename element_type_trait<pT>::element_type
        max1 (typename element_type_trait<pT>::element_type a, typename element_type_trait<pT>::element_type b)
        {
            return a > b ? a : b;
        }

        typename element_type_trait<pT>::element_type
        min1 (typename element_type_trait<pT>::element_type a, typename element_type_trait<pT>::element_type b)
        {
            return a < b ? a : b;
        }
      ...
      private:
        typename element_type_trait<pT>::element_type
        uni_rand (typename element_type_trait<pT>::element_type x1, typename element_type_trait<pT>::element_type x2)
        {
      ...
      public:
        ...
        : n1 (_n1), n2 (_n2), n0 (n1 * n2 + n1), p_fo (0), p_optimizer (0), threshold (1e-4),
          decay_factor (.999), samples (n1 * n2 + n1)
        {
        }

        aga_method ()
        : n1 (50), n2 (20), n0 (n1 * n2 + n1), p_fo (0), p_optimizer (0), threshold (1e-4),
          decay_factor (.999), samples (n1 * n2 + n1)
        {
        }
        ...
        : n1 (rhs.n1), n2 (rhs.n2), n0 (rhs.n0), p_fo (rhs.p_fo), p_optimizer (rhs.p_optimizer),
          threshold (rhs.threshold), decay_factor (rhs.decay_factor), samples (rhs.samples)
        {
        }
                    ...
                    for (size_t j = 0; j < get_size (p); ++j)
                        {
                            set_element (samples[i].p, j,
                                         uni_rand (get_element (lower_bound, j), get_element (upper_bound, j)));
                        }
                }
        }
                    ...
                    for (size_t j = 0; j < get_size (p); ++j)
                        {
                            ...
                            set_element (p, j,
                                         get_element (p, j) + uni_rand (-get_element (reproduction_box, j),
                            ...
                            for (size_t k = 0; k < get_size (p); ++k)
                                {
                                    set_element (samples[i * n2 + j + n1].p, k,
                                                 (get_element (samples[i].p, k) + get_element (buffer[j], k)) / 2.);
            ...
            for (size_t i = 0; i < get_size (reproduction_box); ++i)
                {
                    ...
                    set_element (reproduction_box, i, (get_element (ub, i) - get_element (ub, i)) / n_per_dim);
                }
            return true;
        }
            ...
            for (size_t i = 0; i < get_size (lower_bound); ++i)
                {
                    ...
                    set_element (reproduction_box, i,
                                 (get_element (upper_bound, i) - get_element (lower_bound, i)) / n_per_dim);
                }
