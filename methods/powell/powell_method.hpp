...
namespace opt_utilities
{
     ...
     */
    template <typename rT, typename pT> class powell_method : public opt_method<rT, pT>
    {
      public:
      ...
      private:
        ...
        void powell (array1d_type &p, const T ftol, int &iter, T &fret)
        {
            const int ITMAX = 200;
...
#pragma omp parallel for
                    ...
                    if (fptt < fp)
                        {
                            t = T (2.) * (fp - T (2.) * fret + fptt) * sqr (T (fp - fret - del)) -
                                del * sqr (T (fp - fptt));
                            if (t < T (0.))
                                {
                                    linmin (p, xit, fret, *p_fo);
...
#pragma omp parallel for
      ...
      public:
        powell_method () : threshold (1e-4), xi (NULL_PTR), xi_1d (NULL_PTR)
        {
        }
        ...
        : opt_method<rT, pT> (rhs), p_fo (rhs.p_fo), p_optimizer (rhs.p_optimizer),
          start_point (rhs.start_point), end_point (rhs.end_point), ncom (rhs.ncom),
